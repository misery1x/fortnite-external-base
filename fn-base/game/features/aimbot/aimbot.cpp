...
uintptr_t NetConnection = 0x520;
uintptr_t RotationInput = NetConnection + 0x8;

void AIMBOT_ENGINE::write_memory_event(FVector New_Position)
{
	WriteMemory<FVector>(cache.PlayerController + RotationInput, New_Position);
}
...
void AIMBOT_ENGINE::MoveMouseToEntity(FVector2D Index)
{
	...
	write_memory_event(FVector(-Target.y / AimSpeedY, Target.x / AimSpeedX, 0));
}